...
/**
 * Phase 3 common metrics, ported from gateway-mvc-blocking-spring5 (P3-A)'s GatewayMetrics
 * unchanged except for one deliberate structural difference (Unit 3 §22): P3-A's
 * outbound.blocking.executor.* gauges/counter/timer are dropped entirely rather than kept as
 * always-zero placeholders — P3-B has no blocking outbound executor, and "no metric" is the
 * honest signal for "this resource does not exist here", not a Gauge pinned at 0. Everything else
 * (common gateway.* metrics, servlet.write.* shared metrics) is identical to P3-A's version —
 * see docs/decisions/phase3-metrics-contract.md §3/§4-1.
 */
@Component
