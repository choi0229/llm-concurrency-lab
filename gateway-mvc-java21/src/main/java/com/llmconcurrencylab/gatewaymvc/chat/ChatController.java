...
/**
 * Phase 2 port of Phase 1's ChatController (gateway-mvc-executor-java8).
 * Baseline relay: Front -> Spring MVC -> AsyncContext -> Chat Executor ->
 ...
 * executor's task blocks for the entire upstream call — see
 * docs/test-plan/phase2-design.md section 1 for why this lifecycle is kept
 * byte-for-byte identical across P-E/VT-Limited/VT-Unlimited.
 *
 * Changes from the Phase 1 (Java 8 / javax.servlet) version — see
 * docs/decisions/version-compatibility.md section 6 for the javax->jakarta
 * rationale:
 *   - javax.servlet.* -> jakarta.servlet.* (Tomcat 11 / Servlet 6.1 baseline)
 *   - No recordQueueWait() / executor_queue_wait_seconds — P-E's
 *     SynchronousQueue has zero queue residence by construction (Unit 2
 *     pre-fix 3, docs/test-plan/phase2-design.md section 2-1).
 * Everything else (outcome accounting, timeout semantics, deadline checks,
 * client-disconnect detection) is unchanged from Phase 1.
 *
 * Unit 3: submission goes through ChatTaskSubmitter (not a raw ExecutorService
 * directly) — this is THE seam where THREAD_MODE-specific admission control
 * and instrumentation live (PlatformTaskSubmitter for P-E,
 * VirtualLimitedTaskSubmitter for VT-Limited). ChatController itself has NO
 * THREAD_MODE conditional anywhere — trySubmit()'s boolean return is the only
 * thing it reacts to, and the rejected-path handling below (503 + outcome=
 * "rejected") is identical regardless of which mode rejected the request
 * (docs/test-plan/phase2-design.md Unit 3 note 6 / section 2).
 */
@RestController
@RequestMapping("/chat")
public class ChatController {
    ...
    @Autowired
    public ChatController(ChatTaskSubmitter taskSubmitter, MockLlmClient mockLlmClient, GatewayMetrics metrics) {
    ...
    public void stream(HttpServletRequest request, @RequestBody(required = false) String body) throws IOException {
        ...
        // is what makes it comparable to client_ttfc_seconds (see Phase 1 H1-c).
        ...
        boolean submitted = taskSubmitter.trySubmit(work);
        if (!submitted) {
            // Uniform rejected-path handling regardless of THREAD_MODE — P-E's
            // ThreadPoolExecutor.AbortPolicy and VT-Limited's Semaphore.
            // tryAcquire() failure both land here via ChatTaskSubmitter's
            // boolean contract (docs/test-plan/phase2-design.md Unit 3
            // section 2). Each submitter already incremented its own
            // mode-specific diagnostic counter before returning false.
            log.warn("[{}] rejected: chat task admission failed", requestId);
            try {
                ...
                // Rejected pre-accept: the task was never submitted, so it is
                // excluded from "accepted" by definition (docs/decisions/
                // request-outcome-accounting.md) even though
    ...
    private void relay(AsyncContext asyncContext, String requestBody, String requestId, AsyncRequestState state,
        ...
        if (state.isReleased() || remainingNanosAtStart <= 0) {
            ...
            // releaseOnce call below is then a no-op. If it's false (our own
            // clock tripped a hair before the container's), we leave the
            // AsyncContext alone; the container's own onTimeout will still
            // fire shortly and do the release.
            return;
        }
        ...
        try {
            ...
            // while this thread is already mid-relay. state.isReleased()
            // disambiguates the known race from a genuine bug so the two are
            // never counted under the same metric.
            if (state.isReleased()) {
                metrics.staleTaskSkippedTotal.inc();
