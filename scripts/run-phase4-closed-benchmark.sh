#!/usr/bin/env bash
# Phase 4 Unit 4 — Closed-model harness verification (and, unmodified, the base for Unit 5+
# screening). Fresh Gateway + fresh Mock + fresh Prometheus + k6 closed wave + resource sampler +
# drain + collector + validity gate + cleanup, for exactly ONE model per invocation.
#
# Usage: run-phase4-closed-benchmark.sh <model:m1|m2|m3> <concurrency> <run_label>
#
# This script does NOT decide capacity/scalability -- it is measurement-pipeline plumbing. Unit 4
# uses it at N=20 only; Unit 5 reuses it unmodified for the geometric screening sweep.
set -uo pipefail
...
OUT_DIR="${ROOT}/docs/test-results/phase4/unit4-closed-harness/${MODEL}-n${N}-${RUN_LABEL}"
mkdir -p "$OUT_DIR"
...
# ---- Preflight ----
log "=== Preflight: $MODEL N=$N ($RUN_LABEL) ==="
...
if [ "$MODEL" = "m1" ]; then
  ...
  WEBCLIENT_MAX_CONNECTIONS=800 WEBCLIENT_PENDING_ACQUIRE_MAX_COUNT=800 \
  CHAT_TOTAL_TIMEOUT_MS=$CHAT_TOTAL_TIMEOUT_MS MOCK_LLM_BASE_URL="http://127.0.0.1:${MOCK_PORT}" SERVER_PORT=$GW_PORT \
    nohup "${JAVA_HOME}/bin/java" -jar "$GW_JAR" > "$OUT_DIR/gateway.log" 2>&1 &
fi
GW_PID=$!
...
# Unit 4.6: this is the "before" side of the before/after delta used to attribute server-side
# outcome counters to the wave rather than the 5 prewarm requests (docs/test-plan/
# phase4-closed-harness-protocol.md prewarm-contamination section).
...
log "=== Done: $MODEL N=$N ($RUN_LABEL) -> $OUT_DIR ==="
