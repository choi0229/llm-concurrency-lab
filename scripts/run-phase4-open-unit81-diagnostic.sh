#!/usr/bin/env bash
# Phase 4 Open Unit 8.1 -- M2 R=256 Two-Leg Transport / Ephemeral-Port Forensics DIAGNOSTIC harness.
#
# Forked from scripts/run-phase4-open-diagnostic-r160.sh (frozen, untouched). DIFFERENCES, all
# observation-only (Unit 8.1 governing instruction section 2):
#   * RATE fixed at 256; warmup=60s / measurement=120s unchanged.
#   * Output under docs/test-results/phase4/unit8.1-m2-r256-two-leg-forensics/<subdir>/ .
#   * Replaces the single-leg bash socket sampler with scripts/sample_two_leg_socket_telemetry.py
#     (endpoint-aware; for target=m2 it samples BOTH LEG A k6->:18102 and LEG B Gateway->:8000, and
#     preserves full raw socket rows + per-leg unique-source-port set algebra).
#   * Adds scripts/sample_lsof_ownership.sh -- low-frequency (5s) process-ownership spot-check.
#   * Records the samplers' own CPU (ps) so section 5's "did the instrumentation perturb the
#     measurement" question can be answered from data, not assumed.
# NOT canonical. Never promoted to a Screening/Formal point. No auto-retry (section 28): run once,
# preserve artifacts, stop.
#
# Usage: run-phase4-open-unit81-diagnostic.sh <target:direct-mock|m2> <run_label>
set -uo pipefail
...
CHUNK_COUNT=35
OPEN_TOMCAT_MAX_CONNECTIONS=3200   # frozen (Unit 7.5 final) -- do not change (section 2/section 13)

case "$TARGET" in
  direct-mock)
    TARGET_PORT=$MOCK_PORT
    TARGET_URL="http://127.0.0.1:${MOCK_PORT}/mock/stream"
    SUBDIR="direct-mock-r256"
    LEGS="DIRECT:${MOCK_PORT}"
    ;;
  m2)
    ...
    SUBDIR="m2-r256-two-leg"
    LEGS="A:${GW_PORT},B:${MOCK_PORT}"
    ;;
  *) echo "FATAL: unknown target '$TARGET' (expected direct-mock|m2)" >&2; exit 1 ;;
esac

OUT_ROOT="${ROOT}/docs/test-results/phase4/unit8.1-m2-r256-two-leg-forensics"
...
log "=== Unit 8.1 DIAGNOSTIC target=$TARGET R=$RATE ($RUN_LABEL) warmup=${WARMUP_SEC}s measurement=${MEASUREMENT_SEC}s ==="

# read-only host baseline (section 7)
{
  echo "epoch_s=$(date +%s)"
  echo "portrange.first=$(sysctl -n net.inet.ip.portrange.first 2>/dev/null)"
  echo "portrange.last=$(sysctl -n net.inet.ip.portrange.last 2>/dev/null)"
  echo "portrange.size=$(( $(sysctl -n net.inet.ip.portrange.last) - $(sysctl -n net.inet.ip.portrange.first) + 1 ))"
  echo "tcp.msl_ms=$(sysctl -n net.inet.tcp.msl 2>/dev/null)"
  echo "kern.ipc.somaxconn=$(sysctl -n kern.ipc.somaxconn 2>/dev/null)"
  echo "kern.maxfilesperproc=$(sysctl -n kern.maxfilesperproc 2>/dev/null)"
...
if [ "$TARGET" = "m2" ]; then
  ...
  ACTUAL_CMD=$(ps -o command= -p "$GW_PID" 2>/dev/null)
  {
    ...
    if echo "$ACTUAL_CMD" | grep -q "$GW_JAR_GLOB"; then echo "PID_MATCH=true"; else echo "PID_MATCH=false"; fail "Gateway PID $GW_PID command does not contain $GW_JAR_GLOB"; fi
...
# ---- resource sampler for the process under test (Gateway for m2, Mock for direct-mock) ----
...
[ "$TARGET" = "m2" ] && curl -sS "http://127.0.0.1:${GW_PORT}/actuator/prometheus" > "$OUT_DIR/gateway-metrics-before-wave.txt"

# Final VU formula (docs/decisions/phase4-open-loadgen-vu-sizing.md), identical to canonical harness.
...
# ---- k6 resource sampler (1 Hz) -- to compare loadgen CPU/RSS pressure vs the Direct Mock run ----
echo "epoch_s,rss_kib,pcpu" > "$OUT_DIR/k6-rss-samples.csv"
( while kill -0 "$K6_PID" 2>/dev/null; do
    kr=$(ps -o rss= -p "$K6_PID" 2>/dev/null | tr -d ' ')
    kc=$(ps -o %cpu= -p "$K6_PID" 2>/dev/null | tr -d ' ')
    echo "$(date +%s),${kr:-0},${kc:-0}" >> "$OUT_DIR/k6-rss-samples.csv"
    sleep 1
  done ) &
K6_SAMPLER_PID=$!

# ---- two-leg endpoint-aware socket sampler (1 Hz) ----
python3 "$TWO_LEG_SAMPLER" \
  --k6-pid "$K6_PID" \
  --legs "$LEGS" \
...
# ---- low-frequency lsof ownership spot-check (5 s) ----
"$LSOF_SAMPLER" "$K6_PID" "${GW_PID:--}" "${GW_PORT:-0}" "$MOCK_PORT" \
  "$OUT_DIR/lsof-ownership-samples.txt" 5 &
LSOF_PID=$!

# ---- sampler-overhead recorder (section 5): the samplers' OWN cpu/rss, 2 s cadence ----
echo "epoch_s,two_leg_pcpu,two_leg_rss_kib,lsof_pcpu,lsof_rss_kib,rsssampler_pcpu" > "$OUT_DIR/sampler-overhead.csv"
( while kill -0 "$K6_PID" 2>/dev/null; do
    tlc=$(ps -o %cpu= -p "$TWO_LEG_PID" 2>/dev/null | tr -d ' '); tlr=$(ps -o rss= -p "$TWO_LEG_PID" 2>/dev/null | tr -d ' ')
    lsc=$(ps -o %cpu= -p "$LSOF_PID" 2>/dev/null | tr -d ' '); lsr=$(ps -o rss= -p "$LSOF_PID" 2>/dev/null | tr -d ' ')
    rsc=$(ps -o %cpu= -p "$SAMPLER_PID" 2>/dev/null | tr -d ' ')
    echo "$(date +%s),${tlc:-0},${tlr:-0},${lsc:-0},${lsr:-0},${rsc:-0}" >> "$OUT_DIR/sampler-overhead.csv"
    sleep 2
  done ) &
OVH_PID=$!
...
[ "$TARGET" = "m2" ] && curl -sS "http://127.0.0.1:${GW_PORT}/actuator/prometheus" > "$OUT_DIR/gateway-metrics-final.txt"
...
cat > "$OUT_DIR/environment.json" <<EOF
{
  ...
  "legs": "$LEGS",
  "mock_port": $MOCK_PORT,
  "gateway_port": $( [ "$TARGET" = "m2" ] && echo "$GW_PORT" || echo "null" ),
  ...
  "k6_pid": $K6_PID,
  "gateway_pid": $( [ "$TARGET" = "m2" ] && echo "$GW_PID" || echo "null" ),
  "mock_pid": $MOCK_PID
}
EOF
...
log "=== Done: Unit 8.1 DIAGNOSTIC target=$TARGET R=$RATE ($RUN_LABEL) -> $OUT_DIR ==="
