#!/usr/bin/env bash
# Phase 4 Unit 3 — Direct Mock closed-model control calibration.
#
# k6/xk6-sse -> Mock LLM DIRECTLY (no Gateway in the loop at all). Purpose: confirm the
# measurement environment itself (Mock + k6 + this macOS host) can generate/absorb the load range
# Phase 4 scalability screening will need — NOT a Gateway measurement (docs/decisions/
# phase4-resource-safety-policy.md section 4, docs/test-plan/phase4-design.md Unit 3 brief section
# "매우 중요 — Unit 3의 목적"). Sequential geometric levels, one wave per level, no repeats (this
# is calibration, not Formal). Stops at the first non-clean level — does not auto-continue to
# higher N after a failure.
set -uo pipefail
...
OUT_ROOT="${ROOT}/docs/test-results/phase4/unit3-control-calibration/closed"
...
LEVELS=(100 200 400 800 1600 3200 5000)
...
start_mock() {
  ...
  MOCK_LLM_MAX_CONCURRENT_PROCESSING=0 MOCK_LLM_MAX_WAITING=0 \
    ...
    > /tmp/phase4-unit3-mock.log 2>&1 &
  echo $! > /tmp/phase4-unit3-mock.pid
  cd "$ROOT"
...
stop_mock() {
  if [ -f /tmp/phase4-unit3-mock.pid ]; then
    kill "$(cat /tmp/phase4-unit3-mock.pid)" 2>/dev/null
    for _ in $(seq 1 20); do
      port_free "$MOCK_PORT" && break
      sleep 0.5
    done
    rm -f /tmp/phase4-unit3-mock.pid
  fi
}
...
for N in "${LEVELS[@]}"; do
  log "=== Closed control N=$N ==="
  LEVEL_DIR="$OUT_ROOT/n${N}"
  mkdir -p "$LEVEL_DIR"
  ...
  start_mock
  MOCK_PID=$(cat /tmp/phase4-unit3-mock.pid)
  ...
  cp /tmp/phase4-unit3-mock.log "$LEVEL_DIR/mock.log"

  # Validity extraction from k6 summary JSON.
...
EOF
  ...
  # Watch-only degradation flag (docs/test-plan/phase4-design.md Unit 3 brief section 8: "TTFC/
  # duration p95가 baseline에서 비정상적으로 폭증하지 않음"). Baseline ~1.0s TTFC / ~7.9s duration
  # (Phase 1-3 repeated measurement). Thresholds here (3x TTFC, 1.5x duration) do NOT reuse the
  # Phase 4 MSC SLO (2.0s/10.0s) per that same section's explicit instruction not to. This does
  # NOT stop the loop by itself (not a hard failure) — recorded for post-hoc classification.
...
echo
echo "=== Closed control calibration finished ==="
echo "Last clean N: $LAST_CLEAN"
echo "Stop reason: ${STOP_REASON:-completed all levels clean}"
echo "$LAST_CLEAN" > "$OUT_ROOT/last-clean-n.txt"
echo "${STOP_REASON:-completed all levels clean}" > "$OUT_ROOT/stop-reason.txt"
