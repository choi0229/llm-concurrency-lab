...
SAFE_CLOSED_MAX = 1120  # Phase 4.1 re-freeze: SAFE_CLOSED_MAX_EXTENDED (docs/test-results/phase4/unit6.5-extended-closed-control/SUMMARY.md). Original collect_phase4_closed_result.py keeps 640 unchanged.
...
# Phase 4.1 (Unit 6.6/6.7) fork of collect_phase4_closed_result.py -- the ONLY change from the
# original (diff this file against it to confirm) is this constant. The original stays frozen at
# 800 for any future re-verification of the Unit 5/6 Closed Formal dataset (N<=640);
# this copy is paired exclusively with scripts/run-phase4-closed-benchmark-extended.sh and used
# only for Extended Closed screening/Formal runs (N>640), per the required re-freeze sequence in
# docs/decisions/phase4-scalability-definition.md section 16-5 and
# docs/test-results/phase4/unit6.5-extended-closed-control/SUMMARY.md (SAFE_CLOSED_MAX_EXTENDED=1120
# -> EXTENDED_M3_MAX_CONNECTIONS=1400). Unit 5.3.2's original closure comment (a runtime value that
# disagrees with the frozen environment config is a safety signal, not something to silently trust)
# applies identically here, just against the new frozen value.
M3_FROZEN_WEBCLIENT_MAX_CONNECTIONS = 1400
